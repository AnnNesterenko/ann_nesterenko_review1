...
parser.add_argument('--model', type=str, help="Way to file with model for generation")
...
def generate_sentence(args):
    ...
    while i < args.length and current_word in _model.keys():
        ...
        i += 1
    if i < args.length:
        print("Не получилось построить слово заданной длины, цепочка оборвалась раньше :(")
